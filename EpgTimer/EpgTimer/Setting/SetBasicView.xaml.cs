...
namespace EpgTimer.Setting
{
    ...
    public partial class SetBasicView : UserControl
    {
        ...
        public SetBasicView()
        {
            ...
            // バージョン文字列を表示
            try { textBlock_versionText.Text = ((AssemblyInformationalVersionAttribute)Attribute.GetCustomAttribute(Assembly.GetEntryAssembly(), typeof(AssemblyInformationalVersionAttribute))).InformationalVersion; }
            catch { textBlock_versionText.Text = App.VERSION_TEXT; }
            stackpanel_versionInfo.Visibility = string.IsNullOrEmpty(textBlock_versionText.Text) ? Visibility.Hidden : Visibility.Visible;
